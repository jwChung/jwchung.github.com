...
A 메소드는 특정 기능 수행의 진입점 역할을 한다. 테스트하기 어려운 IO 관련 코드를 담고 있어야 함으로 그 자신 역시 테스트하기 어려운 형태가 되는 특징이 있다. 이런 메소드 또는 함수를 Function Root 라 하자.
...
모든 코드는 테스트되어야 한다. 자동화 테스트가 어려우면, 텍스트를 입력하고 버튼을 눌러 수동으로라도 확인하게 된다. 다음 글에서는 테스트하기 어려운 코드룰 어떻게 처리하는 가에 대해 얘기해보려 한다.
