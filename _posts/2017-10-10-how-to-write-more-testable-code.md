...
모든 코드는 테스트된다. 자동화 테스트가 어려우면, 텍스트를 입력하고 버튼을 눌러 수동으로라도 테스트하게 된다. 다음 글에서는 테스트하기 어려운 코드는 어떻게 처리되는 것이 좋을지에 대해 알아보자.
