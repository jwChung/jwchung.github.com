...
예전에 회사 후배가 어떤 코드를 테스트하고 싶은데, 아직 경험이 많이 부족해서 힘들다란 말을 했다. 그 후배가 테스트하고자 하는 코드(System Under Test, 이하 SUT)를 보니 테스트하기 어려운 IO 로직에 직접 의존하고 있어 테스트가 거의 불가능했다. 수동테스트 밖에 길이 없었던 것이었다. 

테스트에 어려움을 느끼는 초심자의 큰 오해 중 하나는, 그 어려움이 테스트 기법과 스킬 부족 때문이라고 생각하는 것이다. 테스트 스킬이 중요하지 않은 것은 아니지만, 그보다 SUT가 얼마나 테스트하기 쉽게 디자인되었는지가 더 중요한 요소이다. 앞서 후배의 예처럼 IO 로직과 같은 테스트 하기 어려운 코드를 직접 참조해버리면 아무리 뛰어난 스킬이 있더라도 손과 눈으로 수동테스트 하는 방법 밖엔 없는 것이다.
...
글의 순서는 다음과 같다. 될 수 있으면 순서대로 읽어주길 바라며, 기술의 편이를 위해 경어는 생략했다.  각 글에서 목차와 같은 네비게이터가 없다. 글 순서가 헷깔린다면 이 페이지로 돌아와서 순서를 확인해주길 바란다.
