...
중요한 사실은 비용이 높은 경우이건 또는 테스트가 불가능한 경우이건, 우리는 자동화테스트를 이행할 수 없는 경우에 놓일 수 있다는 것이다. 내가 테스트 초심자였을 때 테스트하기 어려운 코드를 만나면, 어떻게 테스트 비용을 낮춰 자동화할 수 있을지만 고민했었다. 테스트하지 말아야 한다는 생각을 못했다. 그러던 중 만난 [xUnit Patterns 책](https://www.amazon.com/xUnit-Test-Patterns-Refactoring-Code/dp/0131495054)에선 [다음과 같은 사실을 일깨워 주었다. 목마른 사막에서 만난 오아시스였다.](http://xunitpatterns.com/Humble%20Object.html)

> 테스트가 어려우면 테스트하지마라. 단, 그 코드는 최대한 겸손(humility)해야 한다.

위 문장에서 테스트하지 말라는 것은 자동화테스트엔 한해서다. 눈으로 확인하는 수동테스트까지 하지말라는 뜻은 아니다. 나아가 겸손의 의미는, 테스트할 수 없는 코드가 테스트 가능한 코드를 가져서는 안되다는 것이다.

사용자 정보(이메일과 비밀번호)를 가지고 회원가입하는 아래 `SignUp` 메소드를 살펴보자. C#으로 작성된 코드이며, `...` 표시는 코드가 생략되었음을 의미한다. 여기서 사용자 정보 저장(`UserStore.AddAsync`)이 테스트 비용이 높아 테스트할 수 없는 경우라 가정해보자. `UserStore.AddAsync`를 테스트할 수 없기 때문에 `SignUp` 도 테스트하기 어렵다. _테스트하기 어려우면 테스트하지 말자._ 대신 `SignUp` 메소드를 겸손하게 만들자.
...
테스트 가능한 코드를 최대한 `SingUp` 에서 분리시켜 겸손하게 만들자. [이전 포스팅에서 테스트 가능한 코드를 작성하려면, 테스트하기 어려운 코드와 쉬운 코드 분리하자고 한 것과 연결된다.](/how-to-write-more-testable-code#테스트하기-어려운-코드와-쉬운-코드-분리) [이전 포스팅의 `Email` 클래스](/how-to-write-more-testable-code#테스트하기-어려운-코드와-쉬운-코드-분리) 와 이와 같은 유형의 `Password` 클래스를 이용해보자. 이 두 클래스를 사용해 아래와 같은 코드를 작성할 수 있다. `SignUp` 자체를 테스트할 순 없지만, 이메일클래스와 비밀번호클래스는 이와 분리시켜 테스트할 수 있다. `SignUp` 메소드를 겸손하게 만든 결과다.
...
모든 코드를 테스트할 순 없다. 비용이 높아서 그럴 수도 있고, 테스트가 아예 불가능할 수도 있다. 테스트할 수 없는 코드는 말 그대로 자동화된 검증이 수행되지 않는다. 그 대신 겸손해야 한다. 테스트 가능한 코드는 그렇지 않은 코드에서 최대한 분리되어야 한다는 것이다. 테스트가 어려운 UI 코드(View)에서 테스트 가능한 코드를 최대한 분리하여 Controller, Presenter 또는 ViewModel 에 위치 시키는 것도 같은 이치다.