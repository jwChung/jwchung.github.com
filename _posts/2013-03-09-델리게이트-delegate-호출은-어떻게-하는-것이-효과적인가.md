...
### 도입
윈폼의 `Control.Invoke(Delegate)`메소드에 대한 [MSDN][]을 살펴보면 다음과 같은 내용이 나온다.
...
막연히 `Delegate.Invoke`가 당연히 빠르다라고만 생각을 했었지 어느 정도인가는 그 동안 직접 확인하지 못했었다.
이번 기회에 위 경우와 더불어 이외의 델리게이트 실행경우까지 성능을 비교해 보기로 한다.
...
`Invoke`를 통하여 호출하지 않을 이유가 없다 것이다. 1, 2번 둘의 차이는 큰 의미가 없다. 1번의 호출의 경우 내부적으로 2번의 `Invoke` 메소드를 호출하게 된다.
...
물론 **모든 경우에서 실행 메소드가 두개일 경우 비슷한 성능을 보인다**라고 절대 말할 수 없다. 실행메소드가 어떻게 구현되었는가에도 분명 영향이 있다.
